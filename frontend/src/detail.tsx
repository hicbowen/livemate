...
function reviewToDraft(review: OperationReview | null): ReviewDraft {
  return review ? { review_date: inputDate(review.review_date), live_session_id: review.live_session_id, summary: review.summary, strengths: review.strengths, observations: review.observations, conclusion: review.conclusion } : emptyReviewDraft()
}

function ReviewFormDialog({ open, anchorId, sessions, initial, onClose, onSaved }: { open: boolean; anchorId: number; sessions: LiveSession[]; initial: OperationReview | null; onClose: () => void; onSaved: () => void }) {
  const [draft, setDraft] = useState<ReviewDraft>(emptyReviewDraft)
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  useEffect(() => { setDraft(reviewToDraft(initial)); setError('') }, [initial, open])
...
function PlanFollowups({ plan, sessions, onAdd, onEdit, onRefresh }: { plan: ImprovementPlan; sessions: LiveSession[]; onAdd: () => void; onEdit: (followup: PlanFollowup) => void; onRefresh: () => void }) {
...
export function AnchorDetailPage({ anchorId, refreshKey, onBack }: { anchorId: number; refreshKey: number; onBack: () => void }) {
  ...
  const openReview = (review: OperationReview | null = null) => { setEditingReview(review); opened('review') }
  ...
  return <AppPage title={anchor.nickname} description={`${anchor.platform}${anchor.category ? ` · ${anchor.category}` : ''} · 最近更新 ${formatDateTime(anchor.updated_at)}`} actions={<><AppButton appearance="subtle" onClick={load}>刷新</AppButton><AppButton appearance="subtle" onClick={onBack}>‹ 返回主播档案</AppButton></>}>
    ...
    {tab === 'overview' && <Overview detail={detail} onOpenIssue={() => openIssue()} onOpenPlan={() => openPlan()} onOpenGoal={() => openGoal()} onOpenSession={() => openSession()} />}
    {tab === 'sessions' && <SessionsTab sessions={sessions} onCreate={() => openSession()} onEdit={openSession} />}
    ...
    {tab === 'plans' && <PlansTab plans={plans} issues={issues} sessions={sessions} selectedPlanId={selectedPlanId} onSelect={setSelectedPlanId} onCreate={() => openPlan()} onEdit={openPlan} onAddFollowup={(planId) => openFollowup(planId)} onEditFollowup={(planId, followup) => openFollowup(planId, followup)} onRefresh={() => setRevision((value) => value + 1)} />}
    ...
    <ReviewFormDialog open={dialog === 'review'} anchorId={anchorId} sessions={sessions} initial={editingReview} onClose={() => setDialog(null)} onSaved={saved} />
    ...
    {selectedPlan && tab !== 'plans' && <span aria-hidden="true" />}
  </AppPage>
}

function Overview({ detail, onOpenIssue, onOpenPlan, onOpenGoal, onOpenSession }: { detail: AnchorDetail; onOpenIssue: () => void; onOpenPlan: () => void; onOpenGoal: () => void; onOpenSession: () => void }) {
  const sessions = arrayOrEmpty(detail.sessions)
  const issues = arrayOrEmpty(detail.issues)
  const plans = arrayOrEmpty(detail.plans)
  const goals = arrayOrEmpty(detail.goals)
  ...
  return <div className="detail-section"><div className="detail-grid"><AppCard className="summary-panel"><div className="panel-heading"><div><h2>最近直播</h2><p>{latest ? formatDate(latest.session_date) : '暂无直播记录'}</p></div><AppButton size="compact" appearance="subtle" onClick={onOpenSession}>+ 记录</AppButton></div>{latest ? <><SessionMetrics session={latest} /><div className="panel-note">{latest.is_abnormal ? `异常：${latest.abnormal_note || '请补充异常说明'}` : latest.notes || '本场暂无补充备注'}</div></> : <Empty title="还没有直播记录" description="先记下一场直播，播伴会开始计算可用指标。" action={<AppButton size="compact" appearance="primary" onClick={onOpenSession}>新增直播</AppButton>} />}</AppCard><AppCard className="summary-panel"><div className="panel-heading"><div><h2>待推进事项</h2><p>问题和方案需要下一次动作。</p></div></div><div className="summary-list"><button onClick={onOpenIssue}><strong>{openIssues.length}</strong><span>个待处理问题</span><b>›</b></button><button onClick={onOpenPlan}><strong>{activePlans.length}</strong><span>个执行中方案</span><b>›</b></button><button onClick={onOpenGoal}><strong>{currentGoal ? 1 : 0}</strong><span>个进行中目标</span><b>›</b></button></div></AppCard></div><section className="content-section"><div className="section-heading"><div><h2>近 30 天趋势</h2><p>按直播日期聚合；空值会保持为空，不会伪造连续数据。</p></div></div><TrendTable trend={arrayOrEmpty(detail.trend)} /></section><section className="content-section"><div className="section-heading"><div><h2>最近活动</h2><p>快速查看最近发生的直播和运营变化。</p></div></div><Timeline detail={detail} /></section></div>
}

function SessionsTab({ sessions, onCreate, onEdit }: { sessions: LiveSession[]; onCreate: () => void; onEdit: (session: LiveSession) => void }) {
  return <section className="detail-section"><div className="section-heading"><div><h2>直播记录</h2><p>数据按场次保存，指标由原始数据计算而来。</p></div><AppButton appearance="primary" onClick={onCreate}>+ 新增直播</AppButton></div>{sessions.length === 0 ? <Empty title="还没有直播记录" description="记录第一场直播后，再回来做复盘。" action={<AppButton appearance="primary" onClick={onCreate}>新增直播</AppButton>} /> : <div className="session-list">{sessions.map((session) => <AppCard key={session.id} className="session-row"><div className="session-row-header"><div><strong>{formatDate(session.session_date)}</strong><span>{session.source || '未知来源'} · {session.duration_minutes === null ? '时长未知' : `${session.duration_minutes} 分钟`}</span></div><div className="row-actions">{session.is_abnormal && <Badge>异常</Badge>}<AppButton size="compact" appearance="subtle" onClick={() => onEdit(session)}>编辑</AppButton></div></div><SessionMetrics session={session} />{session.notes && <p className="session-note">{session.notes}</p>}</AppCard>)}</div>}</section>
}
...
function PlansTab({ plans, issues, sessions, selectedPlanId, onSelect, onCreate, onEdit, onAddFollowup, onEditFollowup, onRefresh }: { plans: ImprovementPlan[]; issues: AnchorIssue[]; sessions: LiveSession[]; selectedPlanId: number | null; onSelect: (id: number | null) => void; onCreate: () => void; onEdit: (plan: ImprovementPlan) => void; onAddFollowup: (planId: number) => void; onEditFollowup: (planId: number, followup: PlanFollowup) => void; onRefresh: () => void }) {
  return <section className="detail-section"><div className="section-heading"><div><h2>改进方案</h2><p>方案围绕问题执行，通过跟进记录效果和下一步。</p></div><AppButton appearance="primary" onClick={onCreate} disabled={issues.length === 0}>+ 新增方案</AppButton></div>{plans.length === 0 ? <Empty title={issues.length === 0 ? '先建立问题，再建立方案' : '还没有改进方案'} description={issues.length === 0 ? '改进方案必须归属于问题，避免没有上下文的动作清单。' : '从问题卡片建立第一个方案。'} /> : <div className="plan-list">{plans.map((plan) => <div className={`plan-card${selectedPlanId === plan.id ? ' is-selected' : ''}`} key={plan.id}><div className="plan-card-header"><div><div className="row-title"><span className="plan-mark">↗</span><h3>{plan.title}</h3><Badge>{plan.priority}</Badge><Badge>{plan.status}</Badge></div><span>问题：{issues.find((issue) => issue.id === plan.issue_id)?.title || '已关联问题'} · 开始于 {formatDate(plan.start_date)}</span></div><div className="row-actions"><AppButton size="compact" appearance="subtle" onClick={() => onEdit(plan)}>编辑</AppButton><AppButton size="compact" appearance={selectedPlanId === plan.id ? 'standard' : 'primary'} onClick={() => onSelect(selectedPlanId === plan.id ? null : plan.id)}>{selectedPlanId === plan.id ? '收起跟进' : '查看跟进'}</AppButton></div></div><div className="plan-detail-grid"><div><span>目标</span><p>{plan.objective || '暂无目标'}</p></div><div><span>动作</span><p className="pre-line">{plan.actions || '暂无动作'}</p></div><div><span>指标</span><p>{plan.metric_name || '暂无指标'} {plan.baseline_value === null ? '' : `${formatMetric(plan.baseline_value)} → `}{plan.target_value === null ? '' : formatMetric(plan.target_value)} {plan.metric_unit}</p></div></div>{selectedPlanId === plan.id && <PlanFollowups plan={plan} sessions={sessions} onAdd={() => onAddFollowup(plan.id)} onEdit={(followup) => onEditFollowup(plan.id, followup)} onRefresh={onRefresh} />}</div>)}</div>}</section>
}
