...
function InlineArrow() {
  return <ArrowRightRegular className="inline-icon" aria-hidden="true" fontSize={14} />
}
...
function SessionDetailDialog({ session, onClose, onEdit, onCreateReview }: { session: LiveSession; onClose: () => void; onEdit: () => void; onCreateReview: () => void }) {
  return <Dialog open title="直播场次详情" description="查看本场完整原始数据和由此计算的指标。" onClose={onClose} width={820} actions={<><AppButton appearance="subtle" onClick={onClose}>关闭</AppButton><AppButton appearance="standard" onClick={onCreateReview}>建复盘</AppButton><AppButton appearance="primary" onClick={onEdit}>编辑场次</AppButton></>}>
    <dl className="info-list session-detail-list">
      ...
      <div><dt>粉丝变化</dt><dd>{formatMetric(session.followers_before, 0)} <InlineArrow /> {formatMetric(session.followers_after, 0)} · 新增 {formatMetric(session.followers_gained, 0)}</dd></div><div><dt>流水</dt><dd>{formatYuan(session.revenue_cents)}</dd></div>
...
function IssueDetailDialog({ issue, plans, reviews, sessions, onClose, onEdit, onCreatePlan, onOpenPlan, onOpenReview, onOpenSession }: { issue: AnchorIssue; plans: ImprovementPlan[]; reviews: OperationReview[]; sessions: LiveSession[]; onClose: () => void; onEdit: () => void; onCreatePlan: () => void; onOpenPlan: (planId: number) => void; onOpenReview: (review: OperationReview) => void; onOpenSession: (session: LiveSession) => void }) {
  ...
  return <Dialog open title="问题详情" description="查看问题的事实依据、原因假设和关联改进方案。" onClose={onClose} width={760} actions={<><AppButton appearance="subtle" onClick={onClose}>关闭</AppButton><AppButton appearance="standard" onClick={onEdit}>编辑问题</AppButton><AppButton appearance="primary" icon={<AddRegular aria-hidden="true" fontSize={16} />} onClick={onCreatePlan}>建方案</AppButton></>}>
    ...
    <div className="related-records"><div className="subheading-row"><strong>关联上下文</strong><span>{relatedReview || relatedSession ? '可追溯' : '未关联'}</span></div><div className="detail-dialog-links">{relatedReview ? <button className="list-row" onClick={() => onOpenReview(relatedReview)}><span className="plan-mark"><HistoryRegular aria-hidden="true" fontSize={16} /></span><span className="row-main"><strong>复盘 · {formatDate(relatedReview.review_date)}</strong><small>{relatedReview.summary || '未填写复盘总结'}</small></span><span className="row-arrow"><ChevronRightRegular aria-hidden="true" fontSize={16} /></span></button> : <p className="muted-copy">未关联复盘记录。</p>}{relatedSession ? <button className="list-row" onClick={() => onOpenSession(relatedSession)}><span className="plan-mark"><LiveRegular aria-hidden="true" fontSize={16} /></span><span className="row-main"><strong>直播场次 · {formatDate(relatedSession.session_date)}</strong><small>平均在线 {formatMetric(relatedSession.avg_online, 0)} · 场观 {formatMetric(relatedSession.views, 0)}</small></span><span className="row-arrow"><ChevronRightRegular aria-hidden="true" fontSize={16} /></span></button> : <p className="muted-copy">关联复盘未绑定具体直播场次。</p>}</div></div>
    <div className="related-records"><div className="subheading-row"><strong>关联方案</strong><span>{relatedPlans.length} 个</span></div>{relatedPlans.length === 0 ? <p className="muted-copy">还没有方案，先把问题拆成一个可执行动作。</p> : <div className="compact-list">{relatedPlans.map((plan) => <button className="list-row" key={plan.id} onClick={() => onOpenPlan(plan.id)}><span className="plan-mark"><ArrowUpRightRegular aria-hidden="true" fontSize={16} /></span><span className="row-main"><strong>{plan.title}</strong><small>{plan.status} · 开始于 {formatDate(plan.start_date)} · 跟进 {plan.followup_count} 次</small></span><span className="row-arrow"><ChevronRightRegular aria-hidden="true" fontSize={16} /></span></button>)}</div>}</div>
  </Dialog>
}
...
type AnomalyDecision = '已忽略' | '继续观察' | '已转为问题'

function AnomalyPanel({ anomalies, onConvert, onDecision }: { anomalies: AnomalyCandidate[]; onConvert: (candidate: AnomalyCandidate) => Promise<void>; onDecision: (candidate: AnomalyCandidate, decision: AnomalyDecision) => Promise<void> }) {
  ...
  const decide = async (candidate: AnomalyCandidate, decision: AnomalyDecision) => {
    setDeciding(`${candidate.id}:${decision}`)
    try {
      await onDecision(candidate, decision)
      hide(candidate, decision)
    } catch {
      // The parent records the actionable error in the page-level notice.
    } finally {
      setDeciding('')
    }
  }
  ...
  return <section className="content-section anomaly-panel"><div className="section-heading"><div><h2>可能异常</h2><p>规则只提示数据变化，不代表已经确认原因。</p></div><span className="section-count warning-count">{visible.length}</span></div><div className="anomaly-list">{visible.map((candidate) => <AppCard key={candidate.id} className="anomaly-row"><div className="anomaly-row-main"><div className="row-title"><strong>{candidate.title}</strong><Badge>提示</Badge></div><p>{candidate.evidence}</p><small>检测于 {formatDate(candidate.detected_at)} · 规则：{candidate.rule}</small></div><div className="row-actions"><AppButton size="compact" appearance="subtle" loading={deciding === `${candidate.id}:已忽略`} onClick={() => void decide(candidate, '已忽略')}>忽略</AppButton><AppButton size="compact" appearance="subtle" loading={deciding === `${candidate.id}:继续观察`} onClick={() => void decide(candidate, '继续观察')}>继续观察</AppButton><AppButton size="compact" appearance="primary" loading={converting === candidate.id} onClick={() => void convert(candidate)}>转为问题</AppButton></div></AppCard>)}</div></section>
}
...
type TimelineItem = { date: string; kind: TimelineKind; title: string; note: ReactNode; event?: AnchorEvent }
...
function Timeline({ detail, onEditEvent, onDeleteEvent }: { detail: AnchorDetail; onEditEvent?: (event: AnchorEvent) => void; onDeleteEvent?: (event: AnchorEvent) => void }) {
  ...
  const items = useMemo<TimelineItem[]>(() => {
    ...
    const statusChanges = arrayOrEmpty(detail.status_changes).map((change: StatusChange) => ({ date: change.changed_at, kind: '状态' as const, title: `${change.entity_type === 'issue' ? '问题' : change.entity_type === 'plan' ? '方案' : '目标'}状态变更`, note: <>{change.entity_title || `#${change.entity_id}`} <InlineArrow /> {change.status}</> }))
...
export function AnchorDetailPage({ anchorId, refreshKey, initialTab = 'overview', targetId, onBack }: { anchorId: number; refreshKey: number; initialTab?: DetailTab; targetId?: number; onBack: () => void }) {
  ...
  const convertAnomaly = async (candidate: AnomalyCandidate) => {
    ...
    try {
      ...
      await Service.SetAnomalyDecision({ anchor_id: anchorId, anomaly_id: candidate.id, detected_at: candidate.detected_at, decision: '已转为问题' })
  ...
  const decideAnomaly = async (candidate: AnomalyCandidate, decision: AnomalyDecision) => {
    try {
      await Service.SetAnomalyDecision({ anchor_id: anchorId, anomaly_id: candidate.id, detected_at: candidate.detected_at, decision })
      setAnomalies((current) => current.filter((item) => item.id !== candidate.id))
    } catch (reason) {
      setError(errorMessage(reason))
      throw reason
  ...
  if (loading && !detail) return <AppPage title="主播详情" description="正在读取主播档案…" actions={<AppButton appearance="subtle" icon={<ChevronLeftRegular aria-hidden="true" fontSize={16} />} onClick={onBack}>返回主播档案</AppButton>}><div className="detail-loading"><div className="loading-state">正在读取主播档案…</div></div></AppPage>
  if (!detail) return <AppPage title="主播详情" description="主播档案无法读取。" actions={<><AppButton appearance="subtle" icon={<ChevronLeftRegular aria-hidden="true" fontSize={16} />} onClick={onBack}>返回主播档案</AppButton><AppButton appearance="subtle" icon={<ArrowClockwiseRegular aria-hidden="true" fontSize={16} />} onClick={load}>重试</AppButton></>}><FormError message={error || '主播不存在或已归档。'} /></AppPage>
  ...
  const selectorItems = tabs.map((item) => ({
    key: item.key,
    label: <>{item.label}{item.count !== undefined && <span className="app-selector-count">{item.count}</span>}</>,
    panelId: `anchor-detail-panel-${item.key}`,
  }))
  return <AppPage title={anchor.nickname} description={`${anchor.platform}${anchor.category ? ` · ${anchor.category}` : ''} · 最近更新 ${formatDateTime(anchor.updated_at)}`} actions={<><AppButton appearance="subtle" icon={<ArrowClockwiseRegular aria-hidden="true" fontSize={16} />} onClick={load}>刷新</AppButton><AppButton appearance="subtle" icon={<ChevronLeftRegular aria-hidden="true" fontSize={16} />} onClick={onBack}>返回主播档案</AppButton></>}>
    <div className="detail-header"><div className="detail-avatar">{anchor.nickname.slice(0, 1)}</div><div className="detail-meta"><div className="detail-name-row"><h1>{anchor.nickname}</h1><Badge>{anchor.stage}</Badge><Badge>{anchor.attention_level}</Badge><Badge>{anchor.status}</Badge></div><p>{anchor.name || '未填写真实姓名'} · {anchor.operator_name || '未指定运营负责人'} · UID {anchor.platform_uid || '未填写'}</p><div className="pill-list">{arrayOrEmpty(anchor.tags).map((tag) => <span className="tag-chip" key={tag}>{tag}</span>)}</div></div><div className="detail-actions"><AppButton appearance="primary" icon={<AddRegular aria-hidden="true" fontSize={16} />} onClick={() => openSession()}>新增直播</AppButton><AppButton appearance="standard" icon={<AddRegular aria-hidden="true" fontSize={16} />} onClick={() => openReview()}>新增复盘</AppButton><AppButton appearance="subtle" onClick={() => openEvent()}>记录事件</AppButton></div></div>
    <AppSelectorBar ariaLabel="主播详情导航" className="detail-selector-bar" value={tab} size="small" onValueChange={(value) => setTab(value as DetailTab)} items={selectorItems} />
    {error && <div className="notice notice-error"><span>{error}</span><button className="notice-dismiss" onClick={() => setError('')} aria-label="关闭错误"><DismissRegular aria-hidden="true" fontSize={16} /></button></div>}
    <AppSelectorPanels value={tab} mountStrategy="unmount" motion="directional" className="detail-selector-panels">
      <AppSelectorPanel value="overview" id="anchor-detail-panel-overview"><Overview anchorId={anchorId} detail={detail} anomalies={anomalies} onConvertAnomaly={convertAnomaly} onDecideAnomaly={decideAnomaly} onOpenIssue={() => setTab('issues')} onOpenPlan={() => setTab('plans')} onOpenGoal={() => setTab('goals')} onOpenSession={() => openSession()} /></AppSelectorPanel>
      <AppSelectorPanel value="sessions" id="anchor-detail-panel-sessions"><SessionsTab anchorId={anchorId} refreshToken={revision} sessions={sessions} onCreate={() => openSession()} onEdit={openSession} onCopy={openSessionCopy} onView={openSessionDetail} onCreateReview={(sessionId) => openReview(null, sessionId)} onDelete={(id) => { if (!window.confirm('确认删除这条直播记录？存在关联复盘、跟进或事件时不能删除。')) return; Service.DeleteSession(id).then(() => setRevision((value) => value + 1)).catch((reason) => setError(errorMessage(reason))) }} /></AppSelectorPanel>
      <AppSelectorPanel value="reviews" id="anchor-detail-panel-reviews"><ReviewsTab reviews={reviews} issues={issues} onCreate={() => openReview()} onEdit={openReview} onCreateIssue={(reviewId) => openIssue(null, reviewId)} onViewIssue={openIssueDetail} /></AppSelectorPanel>
      <AppSelectorPanel value="issues" id="anchor-detail-panel-issues"><IssuesTab issues={issues} onCreate={() => openIssue()} onEdit={openIssue} onView={openIssueDetail} onCreatePlan={(issueId) => openPlan(null, issueId)} onChangeStatus={(id, status) => Service.ChangeIssueStatus(id, status).then(() => setRevision((value) => value + 1)).catch((reason) => setError(errorMessage(reason)))} /></AppSelectorPanel>
      <AppSelectorPanel value="plans" id="anchor-detail-panel-plans"><PlansTab plans={plans} issues={issues} sessions={sessions} refreshToken={revision} selectedPlanId={selectedPlanId} onSelect={setSelectedPlanId} onCreate={() => openPlan()} onEdit={openPlan} onDelete={(id) => { if (!window.confirm('确认删除这个方案？有跟进记录时删除会被拒绝。')) return; Service.DeletePlan(id).then(() => setRevision((value) => value + 1)).catch((reason) => setError(errorMessage(reason))) }} onAddFollowup={(planId) => openFollowup(planId)} onEditFollowup={(planId, followup) => openFollowup(planId, followup)} onRefresh={() => setRevision((value) => value + 1)} /></AppSelectorPanel>
      <AppSelectorPanel value="goals" id="anchor-detail-panel-goals"><GoalsTab goals={goals} selectedGoalId={selectedGoalId} onCreate={() => openGoal()} onEdit={openGoal} onChangeStatus={(id, status) => Service.ChangeGoalStatus(id, status).then(() => setRevision((value) => value + 1)).catch((reason) => setError(errorMessage(reason)))} /></AppSelectorPanel>
      <AppSelectorPanel value="timeline" id="anchor-detail-panel-timeline"><section className="detail-section"><div className="section-heading"><div><h2>运营时间线</h2><p>把数据和上下文放在同一条时间线上，回看变化发生的前后。</p></div><AppButton size="compact" appearance="primary" icon={<AddRegular aria-hidden="true" fontSize={16} />} onClick={() => openEvent()}>记录事件</AppButton></div><Timeline detail={detail} onEditEvent={openEvent} onDeleteEvent={removeEvent} /></section></AppSelectorPanel>
    </AppSelectorPanels>
...
function Overview({ anchorId, detail, anomalies, onConvertAnomaly, onDecideAnomaly, onOpenIssue, onOpenPlan, onOpenGoal, onOpenSession }: { anchorId: number; detail: AnchorDetail; anomalies: AnomalyCandidate[]; onConvertAnomaly: (candidate: AnomalyCandidate) => Promise<void>; onDecideAnomaly: (candidate: AnomalyCandidate, decision: AnomalyDecision) => Promise<void>; onOpenIssue: () => void; onOpenPlan: () => void; onOpenGoal: () => void; onOpenSession: () => void }) {
  const sessions = arrayOrEmpty(detail.sessions)
  const issues = arrayOrEmpty(detail.issues)
  const plans = arrayOrEmpty(detail.plans)
  const goals = arrayOrEmpty(detail.goals)
  ...
  return <div className="detail-section"><AnomalyPanel anomalies={anomalies} onConvert={onConvertAnomaly} onDecision={onDecideAnomaly} /><div className="detail-grid"><AppCard className="summary-panel"><div className="panel-heading"><div><h2>最近直播</h2><p>{latest ? formatDate(latest.session_date) : '暂无直播记录'}</p></div><AppButton size="compact" appearance="subtle" icon={<AddRegular aria-hidden="true" fontSize={16} />} onClick={onOpenSession}>记录</AppButton></div>{latest ? <><SessionMetrics session={latest} /><div className="panel-note">{latest.is_abnormal ? `异常：${latest.abnormal_note || '请补充异常说明'}` : latest.notes || '本场暂无补充备注'}</div></> : <Empty title="还没有直播记录" description="先记下一场直播，播伴会开始计算可用指标。" action={<AppButton size="compact" appearance="primary" onClick={onOpenSession}>新增直播</AppButton>} />}</AppCard><AppCard className="summary-panel"><div className="panel-heading"><div><h2>待推进事项</h2><p>问题和方案需要下一次动作。</p></div></div><div className="summary-list"><button onClick={onOpenIssue}><strong>{openIssues.length}</strong><span>个待处理问题</span><ChevronRightRegular aria-hidden="true" fontSize={16} /></button><button onClick={onOpenPlan}><strong>{activePlans.length}</strong><span>个执行中方案</span><ChevronRightRegular aria-hidden="true" fontSize={16} /></button><button onClick={onOpenGoal}><strong>{currentGoal ? 1 : 0}</strong><span>个进行中目标</span><ChevronRightRegular aria-hidden="true" fontSize={16} /></button></div></AppCard></div><TrendPanel anchorId={anchorId} initialTrend={arrayOrEmpty(detail.trend)} /><section className="content-section"><div className="section-heading"><div><h2>最近活动</h2><p>快速查看最近发生的直播和运营变化。</p></div></div><Timeline detail={detail} /></section></div>
}
...
function PlansTab({ plans, issues, sessions, refreshToken, selectedPlanId, onSelect, onCreate, onEdit, onDelete, onAddFollowup, onEditFollowup, onRefresh }: { plans: ImprovementPlan[]; issues: AnchorIssue[]; sessions: LiveSession[]; refreshToken: number; selectedPlanId: number | null; onSelect: (id: number | null) => void; onCreate: () => void; onEdit: (plan: ImprovementPlan) => void; onDelete: (id: number) => void; onAddFollowup: (planId: number) => void; onEditFollowup: (planId: number, followup: PlanFollowup) => void; onRefresh: () => void }) {
  return <section className="detail-section"><div className="section-heading"><div><h2>改进方案</h2><p>方案围绕问题执行，通过跟进记录效果和下一步。</p></div><AppButton appearance="primary" icon={<AddRegular aria-hidden="true" fontSize={16} />} onClick={onCreate} disabled={issues.length === 0}>新增方案</AppButton></div>{plans.length === 0 ? <Empty title={issues.length === 0 ? '先建立问题，再建立方案' : '还没有改进方案'} description={issues.length === 0 ? '改进方案必须归属于问题，避免没有上下文的动作清单。' : '从问题卡片建立第一个方案。'} /> : <div className="plan-list">{plans.map((plan) => <div className={`plan-card${selectedPlanId === plan.id ? ' is-selected' : ''}`} key={plan.id}><div className="plan-card-header"><div><div className="row-title"><span className="plan-mark"><ArrowUpRightRegular aria-hidden="true" fontSize={16} /></span><h3>{plan.title}</h3><Badge>{plan.priority}</Badge><Badge>{plan.status}</Badge></div><span>问题：{issues.find((issue) => issue.id === plan.issue_id)?.title || '已关联问题'} · 开始于 {formatDate(plan.start_date)} · 预计结束 {formatDate(plan.expected_end_date)}</span></div><div className="row-actions"><AppButton size="compact" appearance="subtle" onClick={() => onEdit(plan)}>编辑</AppButton><AppButton size="compact" appearance="subtle" onClick={() => onDelete(plan.id)}>删除</AppButton><AppButton size="compact" appearance={selectedPlanId === plan.id ? 'standard' : 'primary'} onClick={() => onSelect(selectedPlanId === plan.id ? null : plan.id)}>{selectedPlanId === plan.id ? '收起跟进' : '查看跟进'}</AppButton></div></div><div className="plan-detail-grid"><div><span>目标</span><p>{plan.objective || '暂无目标'}</p></div><div><span>动作</span><p className="pre-line">{plan.actions || '暂无动作'}</p></div><div><span>指标</span><p>{plan.metric_name || '暂无指标'} {plan.baseline_value === null ? '' : <>{formatMetric(plan.baseline_value)} <InlineArrow /> </>}{plan.target_value === null ? '' : formatMetric(plan.target_value)} {plan.metric_unit}</p></div></div>{selectedPlanId === plan.id && <PlanFollowups plan={plan} sessions={sessions} refreshToken={refreshToken} onAdd={() => onAddFollowup(plan.id)} onEdit={(followup) => onEditFollowup(plan.id, followup)} onRefresh={onRefresh} />}</div>)}</div>}</section>
}
...
export function SettingsPage() {
  ...
  return <AppPage title="设置" description="播伴的业务数据保存在本机用户配置目录，不依赖云端同步。">
    {loading ? <div className="loading-state">正在读取应用信息…</div> : !info ? <FormError message={error || '应用信息暂时不可用。'} /> : (
      <div className="settings-page">
        <AppSettingsGroup title="数据目录" description="查看本地数据、数据库和日志所在位置。">
          <AppSettingsRow
            title="数据目录"
            control={(
              <div className="settings-row-control">
                <code className="settings-path">{info.data_dir}</code>
                <AppButton size="compact" appearance="subtle" onClick={() => openDirectory('data')}>打开目录</AppButton>
              </div>
            )}
          />
          <AppSettingsRow title="数据库" control={<code className="settings-path">{info.database_path}</code>} />
          <AppSettingsRow
            title="日志目录"
            control={(
              <div className="settings-row-control">
                <code className="settings-path">{info.log_dir}</code>
                <AppButton size="compact" appearance="subtle" onClick={() => openDirectory('logs')}>打开目录</AppButton>
              </div>
            )}
          />
        </AppSettingsGroup>

        <AppSettingsGroup title="备份 / 恢复" description="备份包含数据库、配置和清单；恢复前会自动生成当前数据的安全备份。">
          <AppSettingsRow title="备份目录" control={<code className="settings-path">{info.backup_dir}</code>} />
          <AppSettingsRow
            title="导出备份"
            description="将当前播伴数据导出为 ZIP 文件。"
            control={<AppButton appearance="primary" onClick={exportBackup} loading={busy}>导出备份 ZIP</AppButton>}
          />
          <AppSettingsRow
            title="导入备份"
            description="导入前会自动备份当前数据库。"
            control={(
              <label className="button-like">
                <span>{busy ? '处理中…' : '导入备份 ZIP'}</span>
                <input type="file" accept=".zip,application/zip" disabled={busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void importBackup(file); event.currentTarget.value = '' }} />
              </label>
            )}
          />
        </AppSettingsGroup>

        <AppSettingsGroup title="AI 配置" description="AI 辅助能力尚未接入，当前仅保留配置入口。">
          <AppSettingsRow title="接入状态" control={<Badge>待实现</Badge>} />
          <AppSettingsRow title="服务商" description="后续可在此选择 AI 服务商。" control={<AppSelect options={[{ value: '', label: '暂未配置' }]} value="" disabled />} />
          <AppSettingsRow title="模型与密钥" description="后续可配置模型、API Key 及调用偏好。" control={<span className="settings-placeholder">暂未开放</span>} />
        </AppSettingsGroup>

        <AppSettingsGroup title="主题" description="主题切换尚未实现，当前使用默认浅色主题。">
          <AppSettingsRow title="当前主题" control={<Badge>默认浅色</Badge>} />
          <AppSettingsRow title="主题模式" description="后续支持浅色、深色及跟随系统。" control={<AppSelect options={[{ value: 'light', label: '浅色主题' }, { value: 'dark', label: '深色主题' }, { value: 'system', label: '跟随系统' }]} value="light" disabled />} />
        </AppSettingsGroup>

        {message && <div className="notice notice-success">{message}</div>}
        {error && <FormError message={error} />}

        <AppSettingsGroup title="应用信息" description={info.description}>
          <AppSettingsRow title="产品名称" control={<span>{info.name}</span>} />
          <AppSettingsRow title="项目名" control={<span>{info.project}</span>} />
          <AppSettingsRow title="App ID" control={<code className="settings-path">{info.app_id}</code>} />
          <AppSettingsRow title="版本" control={<Badge>{info.version}</Badge>} />
        </AppSettingsGroup>
      </div>
    )}
  </AppPage>
